package app.huaweiblogplus.providers.social.facebook;
...
import app.huaweiblogplus.providers.social.SocialPost;
import app.huaweiblogplus.providers.social.SocialPostAdapter;
...
public class FacebookFragment extends Fragment implements InfiniteRecyclerViewAdapter.LoadMoreListener, CollapseControllingFragment {
	...
	private static String API_URL_MIDDLE = "/feed/?access_token=";
	private static String API_URL_END = "&date_format=U&fields=comments.limit(50).summary(1),from,picture,message,story,id,created_time,full_picture,attachments{title,url_unshimmed,media,media_type,subattachments}&limit=10";
		...
		try {
			...
			// parsing json object
             ...
             for (int i = 0; i < dataJsonArray.length(); i++) {
				 ...
				 //Log.e("HMS", "onNewToken refresh token:" + photoJson.getJSONObject("comments").toString());
				 // Dataprivacy deactivated
                 //post.commentsJson = photoJson.getJSONObject("comments").toString();
				 post.commentsJson = "{\"data\":[{\"created_time\":1604310857,\"message\":\"Facebook Kommentare sind aktuell leider aus Datenschutzgründen nicht möglich - Wir sind dran!\",\"id\":\"3289569754499054\"}],\"summary\":{\"order\":\"chronological\",\"total_count\":1,\"can_comment\":false}}";
