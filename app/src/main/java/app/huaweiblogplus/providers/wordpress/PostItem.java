/**
 *
 * Workaround for multi installation of the blog - insert replace of domain
 * .replace("https://apiv2.huaweiblog.de","https://www.huaweiblog.de")
 *
 */

package app.huaweiblogplus.providers.wordpress;

import app.huaweiblogplus.attachmentviewer.model.MediaAttachment;
import app.huaweiblogplus.util.SerializableJSONArray;
...
public class PostItem implements Serializable {
    ...
    public String getImageCandidate(){
        ...
        if (getFeaturedImageUrl() != null && !getFeaturedImageUrl().equals(""))
            return getFeaturedImageUrl().replace("https://apiv2.huaweiblog.de","https://www.huaweiblog.de");
        ...
        if (getAttachments().size() > 0 && getAttachments().get(0).getMime().startsWith(MediaAttachment.MIME_PATTERN_IMAGE))
            return getAttachments().get(0).getUrl().replace("https://apiv2.huaweiblog.de","https://www.huaweiblog.de");
        //If there is an attachment with a thumbnail, return it
            ...
            return getAttachments().get(0).getThumbnailUrl().replace("https://apiv2.huaweiblog.de","https://www.huaweiblog.de");

        //If there is a thumbnail, return it
        if (getThumbnailUrl() != null && !getThumbnailUrl().equals("") && !getThumbnailUrl().equals("(null)"))
            return getThumbnailUrl().replace("https://apiv2.huaweiblog.de","https://www.huaweiblog.de");
