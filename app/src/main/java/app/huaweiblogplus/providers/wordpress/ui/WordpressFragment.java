...
import app.huaweiblogplus.providers.wordpress.api.WordpressCategoriesLoader;
...
public class WordpressFragment extends Fragment implements InfiniteRecyclerViewAdapter.LoadMoreListener {
	...
	public void getPosts() {
		Log.e("POSTSARGS", "result:" + arguments[0]);
		Log.e("POSTSARGS", "result:" + arguments[1]);
		Log.e("POSTSARGS", "result:" + arguments[2]);
		if (arguments.length > 1 && !arguments[1].equals("") && arguments[2].equals("category")) {
            //Load category posts
			urlSession = WordpressPostsLoader.getCategoryPosts(mInfo, arguments[1]);
		} else if (arguments.length > 1 && !arguments[1].equals("") && arguments[2].equals("tag")) {
			//Load tag posts
			urlSession = WordpressPostsLoader.getTagPosts(mInfo, arguments[1]);
		} else {
            //Load recent posts
